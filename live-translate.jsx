...
function pcm16ToFloat32(bytes) {
  const view = new DataView(bytes.buffer, bytes.byteOffset, bytes.byteLength); const output = new Float32Array(Math.floor(bytes.byteLength / 2));
...
function downsampleTo16k(buffer, sourceRate) {
  ...
  const ratio = sourceRate / 16000; const length = Math.round(buffer.length / ratio); const result = new Float32Array(length); let offset = 0;
  for (let i = 0; i < length; i += 1) { const next = Math.min(Math.round((i + 1) * ratio), buffer.length); let sum = 0; let count = 0; for (let j = offset; j < next; j += 1) { sum += buffer[j]; count += 1; } result[i] = count ? sum / count : 0; offset = next; }
  return result;
}
function floatToPcm16Base64(samples) {
  const bytes = new Uint8Array(samples.length * 2); const view = new DataView(bytes.buffer);
  for (let i = 0; i < samples.length; i += 1) { const sample = Math.max(-1, Math.min(1, samples[i])); view.setInt16(i * 2, sample < 0 ? sample * 32768 : sample * 32767, true); }
...
async function getToken(targetLanguageCode, echoTargetLanguage) {
  ...
  if (!data.token) throw new Error('The Live Translate token service returned no token.'); return data.token;
}
...
export function LiveTranslate({ onExit }) {
  const [targetLanguage, setTargetLanguage] = useState('en'); const [echoTargetLanguage, setEchoTargetLanguage] = useState(false); const [status, setStatus] = useState('idle'); const [segments, setSegments] = useState([]); const [audioActive, setAudioActive] = useState(false); const [error, setError] = useState(''); const [completedAt, setCompletedAt] = useState(null); const [saved, setSaved] = useState(false);
  const wsRef = useRef(null); const streamRef = useRef(null); const audioContextRef = useRef(null); const processorRef = useRef(null); const sourceRef = useRef(null); const inputBufferRef = useRef(new Float32Array()); const playbackContextRef = useRef(null); const nextPlaybackTimeRef = useRef(0); const stoppedRef = useRef(false); const statusRef = useRef('idle'); const closeSessionRef = useRef(null);
  ...
  const updateTranscript = useCallback((type, text) => {
    ...
    setSegments(current => {
      ...
      if (!last || last[type]) return [...current, { id: makeId(), input: type === 'input' ? text : '', output: type === 'output' ? text : '' }];
      const next = [...current]; next[next.length - 1] = { ...last, [type]: `${last[type] || ''}${text}` }; return next;
    });
  }, []);

  const handleServerMessage = useCallback(async event => {
    try {
      const message = JSON.parse(event.data); const content = message?.serverContent; if (!content) return;
      ...
      for (const part of content.modelTurn?.parts || []) {
        ...
        const bytes = base64ToBytes(part.inlineData.data); const samples = pcm16ToFloat32(bytes);
        if (!playbackContextRef.current) playbackContextRef.current = new (window.AudioContext || window.webkitAudioContext)();
        const context = playbackContextRef.current; if (context.state === 'suspended') await context.resume();
        const audioBuffer = context.createBuffer(1, samples.length, 24000); audioBuffer.copyToChannel(samples, 0);
        const source = context.createBufferSource(); source.buffer = audioBuffer; source.connect(context.destination);
        const start = Math.max(context.currentTime + 0.02, nextPlaybackTimeRef.current || context.currentTime + 0.02); source.start(start); nextPlaybackTimeRef.current = start + audioBuffer.duration; setAudioActive(true);
  ...
  const stopCapture = useCallback(async () => {
    processorRef.current?.disconnect(); sourceRef.current?.disconnect(); processorRef.current = null; sourceRef.current = null;
  ...
  const closeSession = useCallback(async () => {
    stoppedRef.current = true; await stopCapture(); const ws = wsRef.current; wsRef.current = null;
    ...
    nextPlaybackTimeRef.current = 0; setAudioActive(false);
  }, [stopCapture]);
  ...
  const startCapture = useCallback(async () => {
    ...
    const context = new (window.AudioContext || window.webkitAudioContext)(); audioContextRef.current = context; await context.resume();
    const source = context.createMediaStreamSource(stream); sourceRef.current = source; const processor = context.createScriptProcessor(4096, 1, 1); processorRef.current = processor;
    processor.onaudioprocess = event => {
      if (stoppedRef.current || statusRef.current !== 'active') return;
      const input = event.inputBuffer.getChannelData(0); const merged = new Float32Array(inputBufferRef.current.length + input.length); merged.set(inputBufferRef.current); merged.set(input, inputBufferRef.current.length); inputBufferRef.current = merged;
      const targetSamples = Math.floor(context.sampleRate * 0.1);
      while (inputBufferRef.current.length >= targetSamples) { const chunk = inputBufferRef.current.slice(0, targetSamples); inputBufferRef.current = inputBufferRef.current.slice(targetSamples); const pcm = downsampleTo16k(chunk, context.sampleRate); const ws = wsRef.current; if (ws?.readyState === WebSocket.OPEN) ws.send(JSON.stringify({ realtimeInput: { audio: { data: floatToPcm16Base64(pcm), mimeType: 'audio/pcm;rate=16000' } } })); }
    };
    source.connect(processor);
    const silentGain = context.createGain(); silentGain.gain.value = 0; processor.connect(silentGain); silentGain.connect(context.destination);
  }, []);
  ...
  const start = useCallback(async () => {
    ...
    } catch (e) { await closeSession(); setStatus('idle'); setError(e?.message || 'Unable to start translation.'); }
  }, [closeSession, echoTargetLanguage, handleServerMessage, status, targetLanguage, startCapture]);

  const pause = useCallback(async () => {
    if (status === 'active') { await stopCapture(); setStatus('paused'); }
    else if (status === 'paused') { try { stoppedRef.current = false; setStatus('active'); await startCapture(); } catch (e) { setStatus('paused'); setError(e?.message || 'Unable to resume microphone capture.'); } }
  }, [startCapture, status, stopCapture]);
  const end = useCallback(async () => { await closeSession(); setCompletedAt(Date.now()); setStatus('completed'); }, [closeSession]);
  useEffect(() => () => { closeSessionRef.current?.(); }, []);

  const transcript = useMemo(() => segments.map((segment, index) => ({ ...segment, index })), [segments]);
  const saveConversation = useCallback(() => { const item = { id: makeId(), createdAt: completedAt || Date.now(), targetLanguage, targetLanguageName: languageName(targetLanguage), echoTargetLanguage, transcript }; const library = loadLibrary(); localStorage.setItem(STORAGE_KEY, JSON.stringify([item, ...library].slice(0, 100))); setSaved(true); }, [completedAt, echoTargetLanguage, targetLanguage, transcript]);
  const downloadMarkdown = useCallback(() => { const lines = [`# Live Translation`, '', `Date: ${formatDate(completedAt || Date.now())}`, `Target language: ${languageName(targetLanguage)}`, '', '---', '']; transcript.forEach((segment, index) => lines.push(`### Segment ${index + 1}`, '', `**Original:** ${segment.input || '—'}`, '', `**Translation:** ${segment.output || '—'}`, '')); const blob = new Blob([lines.join('\n')], { type: 'text/markdown;charset=utf-8' }); const url = URL.createObjectURL(blob); const a = document.createElement('a'); a.href = url; a.download = `live-translation-${new Date(completedAt || Date.now()).toISOString().slice(0,10)}.md`; a.click(); setTimeout(() => URL.revokeObjectURL(url), 1000); }, [completedAt, targetLanguage, transcript]);
  const downloadPdf = useCallback(async () => { try { const { jsPDF } = await import('jspdf'); const doc = new jsPDF({ unit: 'pt', format: 'a4' }); const margin = 44; const width = 595 - margin * 2; let y = 52; const addText = (text, size = 10, bold = false, gap = 7) => { doc.setFont('helvetica', bold ? 'bold' : 'normal'); doc.setFontSize(size); const lines = doc.splitTextToSize(String(text || '—'), width); if (y + lines.length * (size + 4) > 800) { doc.addPage(); y = 52; } doc.text(lines, margin, y); y += lines.length * (size + 4) + gap; }; addText('Live Translation', 20, true, 12); addText(`Date: ${formatDate(completedAt || Date.now())}`); addText(`Target language: ${languageName(targetLanguage)}`, 10, false, 12); transcript.forEach((segment, index) => { addText(`Segment ${index + 1}`, 11, true, 5); addText(`Original: ${segment.input || '—'}`); addText(`Translation: ${segment.output || '—'}`, 10, false, 12); }); doc.save(`live-translation-${new Date(completedAt || Date.now()).toISOString().slice(0,10)}.pdf`); } catch (e) { setError(e?.message || 'PDF export failed.'); } }, [completedAt, targetLanguage, transcript]);
