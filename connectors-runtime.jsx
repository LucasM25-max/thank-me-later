...
async function chatWithGithub(request) {
  ...
  const toolsResponse = await callGateway('connect', {}); githubSessionId = toolsResponse.sessionId || githubSessionId;
  ...
  for (let i = 0; i < 4; i += 1) {
    ...
    if (!match) break;
    let call; try { call = JSON.parse(match[1]); } catch { break; }
    const allowed = tools.find(t => t.name === call?.name); if (!allowed) break;
    let toolResult;
...
function installFetchInterceptor() { if (installed) return; installed = true; window.fetch = async (input, init = {}) => { const url = typeof input === 'string' ? input : input?.url || ''; if (githubEnabled && url.includes('/api/chat') && init?.body) return chatWithGithub({ body: init.body }); return originalFetch(input, init); }; }

function SidebarConnectorButton({ onOpen }) { useEffect(() => { const find = () => { const library = [...document.querySelectorAll('.sidebar-tool')].find(el => el.textContent?.trim() === 'Library'); if (!library) return; let button = document.getElementById('tml-connectors-sidebar-button'); if (!button) { button = document.createElement('button'); button.id = 'tml-connectors-sidebar-button'; button.className = 'sidebar-tool tml-connectors-sidebar-button'; button.innerHTML = '<span class="sidebar-tool-icon">⌘</span><span>Connectors</span>'; button.addEventListener('click', onOpen); library.parentElement.insertBefore(button, library); } }; find(); const observer = new MutationObserver(find); observer.observe(document.body, { childList: true, subtree: true }); return () => observer.disconnect(); }, [onOpen]); return null; }
function Overlay({ close }) { return <div className="tml-connectors-overlay"><Connectors onExit={close} onConnectorChange={state => { githubEnabled = state.status === 'connected' && state.enabled; }} /></div>; }
function boot() { installStyles(); installFetchInterceptor(); const mount = document.createElement('div'); mount.id = 'tml-connectors-runtime'; document.body.appendChild(mount); let root = null; const open = () => { if (root) return; root = createRoot(mount); root.render(<Overlay close={() => { root?.unmount(); root = null; }} />); }; const helper = document.createElement('div'); helper.style.display = 'none'; document.body.appendChild(helper); createRoot(helper).render(<SidebarConnectorButton onOpen={open} />); window.__tmlConnectors = { setToken: token => { githubToken = token || ''; }, setEnabled: enabled => { githubEnabled = !!enabled; } }; }
