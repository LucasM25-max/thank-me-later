...
export function Connectors({ onExit, onConnectorChange }) {
  ...
  const loadConnection = async () => {
    ...
    if (data) {
      setConnection({ status: data.status === 'connected' ? 'connected' : 'disconnected', enabled: false });
  ...
  return <div className="connectors-page">
    ...
    <div className="connector-card">
      ...
      <div className="connector-actions"><button className={`connector-toggle ${connection.enabled ? 'on' : ''}`} onClick={toggle} disabled={busy}><span><Power size={14}/>{connection.enabled ? 'Enabled' : 'Disabled'}</span><i/></button>{connection.status === 'connected' ? <button className="connector-secondary" onClick={disconnect} disabled={busy}>{busy ? <Loader2 className="spin" size={14}/> : <X size={14}/>} Disconnect</button> : <button className="connector-primary" onClick={() => setShowConnect(true)} disabled={busy}><GitBranch size={15}/> Connect GitHub</button>}</div>
    ...
    {showConnect && <div className="connector-modal-backdrop" onMouseDown={e => e.target === e.currentTarget && setShowConnect(false)}><div className="connector-modal"><button className="connector-modal-close" onClick={() => setShowConnect(false)}><X size={17}/></button><div className="connector-modal-icon"><GitBranch size={21}/></div><h3>Connect GitHub</h3><p>Connect the official GitHub MCP server. Your credential is kept in memory only and is never written to browser storage.</p><label>GitHub access token<input type="password" value={token} onChange={e => setToken(e.target.value)} autoComplete="off" placeholder="Paste token"/></label><button className="connector-primary modal-connect" onClick={connect} disabled={busy}>{busy ? <><Loader2 className="spin" size={15}/> Checking…</> : <><Check size={15}/> Connect</>}</button></div></div>}
  </div>;
}
