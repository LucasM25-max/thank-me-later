export default async function handler(req, res) {
  ...
  try {
    ...
    const modePrompts = {
      ...
      coding: 'Act as an expert software engineer. Produce robust, maintainable solutions, explain important assumptions, and consider edge cases and security.',
    ...
    // ApiBeam is an OpenAI-compatible bridge. Keep this endpoint on the standard
    // chat/completions route so it remains compatible with the Chrome extension.
    ...
    const r = await fetch(`${base}/chat/completions`, {
      ...
      body: JSON.stringify({ model: process.env.APIBEAM_MODEL || model, messages: converted, temperature: 0.7 })
    });
    const data = await r.json();
