...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (model === 'minimax-m3' || model === 'gpt-5.6-terra' || model === 'qwen3.8-max') {
      if (!process.env.POLLINATIONS_API_KEY) return res.status(500).json({ error: 'POLLINATIONS_API_KEY is not configured' });
      const pollinationsModel = model === 'gpt-5.6-terra' ? 'Lorodn4x/gpt-5.6-terra' : model === 'qwen3.8-max' ? 'vendouple/qwen3.8-max' : 'Lorodn4x/minimax-m3';
